...
volatile unsigned int * queueThresholdStatus= (volatile unsigned int *)CSL_QM_SS_CFG_QM_STATUS_RAM_REGS;
CSL_Qm_intdRegs * qmIntRegs= (CSL_Qm_intdRegs *)CSL_QM_SS_CFG_INTD_REGS;

/*PDSP definition*/
volatile unsigned int * qmPdspIRam[2] = 
{
	(volatile unsigned int * )CSL_QM_SS_CFG_APDSP1_RAM_REGS,
	(volatile unsigned int * )CSL_QM_SS_CFG_APDSP2_RAM_REGS
};
...
volatile unsigned int * qmPdspCmd[2]= 
{
	(volatile unsigned int * )CSL_QM_SS_CFG_SCRACH_RAM1_REGS,
	(volatile unsigned int * )CSL_QM_SS_CFG_SCRACH_RAM2_REGS
};
...
volatile unsigned int * qmDmaTxChPriority = 
	(volatile unsigned int *)CSL_QM_SS_CFG_CPPI_DMA_TX_SCHEDULER_CFG_REGS;
...
volatile unsigned int * aifDmaTxChPriority = 
	(volatile unsigned int *)CSL_AIF2_CFG_CPPI_DMA_TX_SCHEDULER_CFG_REGS;
...
volatile unsigned int * srioDmaTxChPriority = 
	(volatile unsigned int *)CSL_SRIO_CONFIG_CPPI_DMA_TX_SCHEDULER_CFG_REGS;
...
volatile unsigned int * paDmaTxChPriority = 
	(volatile unsigned int *)CSL_PA_SS_CFG_CPPI_DMA_TX_SCHEDULER_CFG_REGS;
...
unsigned int linkingEntrySize= 0; 	/*total number of linking entries*/
...
void KeyStone_QMSS_Linking_RAM_init(unsigned long long * linkingRAM1,
	unsigned int linkingRMA1EntrySize)
{
	int i;
...
void KeyStone_QMSS_Descriptor_Regions_init(
	Qmss_DescMemRegionCfg * descMemRegionCfg, unsigned int uiDescRegionNum)
{
	int i;
...
void KeyStone_Host_Descriptor_Queues_init(
	FreeHostQueueCfg *hostQuCfg, unsigned int uiQuCfgNumber)
{
	int i, j;
	unsigned int uiTempletSize, uiBufferAddress, uiDescriptorAddress;
...
void KeyStone_Mono_Descriptor_Queues_init(
	FreeMonoQueueCfg *monoQuCfg, unsigned int uiQuCfgNumber)
{
	int i, j;
	unsigned int uiTempletSize, uiDescriptorAddress;
...
void KeyStone_Qmss_Download_Firmware (
	Qmss_PdspId pdspId, void *image, unsigned int size)
{
...
unsigned int KeyStone_Qmss_Config_Acc_Channel (
	Qmss_PdspId pdspId, Qmss_AccCmdCfg *cfg)
{
    unsigned int         cmd[5];
    volatile unsigned int   *cmdPtr, *reg;
    unsigned int            index;
    ...
    reg = (unsigned int *) ((unsigned char *) qmPdspCmd[pdspId] + 4 * 4);

    /* Write command word last */
    cmdPtr = ((unsigned int *) &cmd) + 4;
    ...
    return (unsigned int) (CSL_FEXTR (*reg, 31, 24));
}

/*This function disables the accumulator functionality for the specified channel*/
unsigned int KeyStone_Qmss_disable_Acc_Channel (
	Qmss_PdspId pdspId, unsigned char channel)
{
    unsigned int         cmd[5];
    volatile unsigned int   *cmdPtr, *reg;
    unsigned int            index;
 ...
 */
unsigned int KeyStone_Qmss_config_Acc_Timer (
	Qmss_PdspId pdspId, unsigned short timerConstant)
{
    ...
    /* Point to the accumulator command register's last word */
    reg = (unsigned int *) ((unsigned char *) qmPdspCmd[pdspId] + 4);
    *reg-- = timerConstant;
 ...
 */
unsigned int KeyStone_Qmss_Config_Reclaim_Queue (
	Qmss_PdspId pdspId, unsigned int uiQueueNumber)
{
    volatile unsigned int   *reg;
    unsigned char             result;
...
void KeyStone_pktDma_Global_Control (
	...
	unsigned int uiStarvationWaitCycles, unsigned int rxPriority, unsigned int txPriority)
{
...
void KeyStone_pktDma_TxCh_config(PKT_DMA_Regs * pktDmaRegs, unsigned int uiChNum,
...
void KeyStone_pktDma_configureRxFlow (
	...
	PktDma_RxFlowCfg * cfg, unsigned int uiFlowNum)
{
	int i;
