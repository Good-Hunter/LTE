...
//extern const unsigned int acc48_le[373];
...
Qmss_Result BCP_initQm()
{
    ...
    qm_init.linkingRAM0Base = 0; //set to QMSS internal LRAM
...
void BCP_initBcp()
{
	...
	tmFlowCfg.format_in 		  =   Bcp_DataFormat_NoChange;//Bcp_DataFormat_NoChange;
	tmFlowCfg.endian_in           =   Bcp_EndianFormat_8;//小端进;
    tmFlowCfg.endian_out          =   Bcp_EndianFormat_8;//大端出
...
void BCP_setupQm()
{
    ...
    /* Setup memory region */
    {
        ...
        tmpMemRegInfo.memRegion     = Qmss_MemRegion_MEMORY_REGION2;//Qmss_MemRegion_MEMORY_REGION0;
        tmpMemRegInfo.startIndex    = NUM_FFTC_HOST_DESC + LL2_HOST_DESC_LINK_ENTRY_SIZE;//0;
        tmpMemRegInfo.manageDescFlag = Qmss_ManageDesc_MANAGE_DESCRIPTOR;//Qmss_ManageDesc_UNMANAGED_DESCRIPTOR;
...
void BCP_ReConfigULEndian()
{
    Bcp_TmFlowEntry tmFlowCfg;

    /* Setup TM flow entry configuration for BCP */
    tmFlowCfg.pkt_type            =   0;      // No special pkt assignments
    tmFlowCfg.dsp_int_sel         =   0;      // Interrupt core 0

    tmFlowCfg.endian_in 		  =   Bcp_EndianFormat_16;
	tmFlowCfg.format_in 		  =   Bcp_DataFormat_NoChange;
    tmFlowCfg.endian_out          =   Bcp_EndianFormat_8;
