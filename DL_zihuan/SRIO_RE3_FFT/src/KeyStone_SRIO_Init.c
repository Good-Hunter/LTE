...
void Keystone_SRIO_Timeout_Config(SRIO_Config * srio_cfg,
	unsigned int logicalRepsonseTimeout_us, 
	unsigned int physicalPortTimeout_us, 
	unsigned int linkInitSilenceTimeout_us)
{
	unsigned int uiTimeout, uiMaxTimeout;
	...
	srioRegs->RIO_PRESCALAR_SRV_CLK= 
		(unsigned int)srio_cfg->serdes_cfg->commonSetup.inputRefClock_MHz/10;
...
void Keystone_SRIO_TX_Queue_Cfg(
	...
	unsigned int uiNumTxQueue)
{
	int i;
	unsigned int uiMask, uiShift, uiRegIndex;

	/*For SRIO, priority 3 is highest, 0 is lowest
	For PktDMA channel, priority 0 is highest, 3 is lowest*/
	unsigned int mapSrioPriToTxChPri[4]={3,2,1,0};
...
void Keystone_SRIO_packet_forwarding_Cfg(
	...
	unsigned int pktForwardingEntryNum)
{
    int i = 0;
	...
	SRIO_Interrupt_Cfg * interrupt_cfg)
{
	...
	volatile unsigned int * ICRR= (volatile unsigned int *)srioRegs->DOORBELL_ICRR;
...
void Keystone_SRIO_Flow_Control(
    ...
    unsigned int uiNumFlowControlID)
{
	int i;
	unsigned int uiFlowMask = 0;
