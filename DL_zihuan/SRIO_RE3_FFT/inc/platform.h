 ...
 *  		unsigned int,  int, unsigned char, int8_t, unsigned short and int16_t. The standard
 *  		C char is used for strings. Complex types (or typdefs) if used are defined
 *  		within this header file.
 */
#include <stdint.h>
#define EXAMPLE_VARIABLE unsigned int
/*@}*/  /* defgroup */
 ...
 *
 *   @return    unsigned int >0  Global L2 address
 *
 */
unsigned int Convert_CoreLocal2GlobalAddr (unsigned int  addr);
...
#define Platform_STATUS         int /** Platform API return type */
...
typedef struct {
    ...
    /**<CPU Revision ID*/
    unsigned short megamodule_revision_major;
    /**<Megamodule Revision ID Major*/
    unsigned short megamodule_revision_minor;
    /**<Megamodule Revision ID Minor*/
    unsigned short silicon_revision_major;
    /**<Silicon Revision ID Major*/
    unsigned short silicon_revision_minor;
...
typedef struct {
    ...
    unsigned char efuse_mac_address[6];
    ...
    unsigned char eeprom_mac_address[6];
...
Platform_STATUS platform_external_memory_test(unsigned int start_address, unsigned int end_address);
...
Platform_STATUS platform_internal_memory_test(unsigned int id);
...
unsigned int platform_get_coreid(void);
...
unsigned int platform_get_switch_state(unsigned int id);
...
Platform_STATUS platform_get_macaddr(PLATFORM_MAC_TYPE type, unsigned char * mac_address);
...
Platform_STATUS platform_get_emac_info(unsigned int port_num, PLATFORM_EMAC_EXT_info * emac_info);
 ...
 */
 ...
 int platform_get_phy_addr(unsigned int port_num);
...
Platform_STATUS platform_phy_link_status(unsigned int port_num);
 ...
 *
 */
typedef unsigned int PLATFORM_DEVHANDLE;
...
typedef struct {
     ...
     int device_id;				/**<Manufacturers device ID*/
    PLATFORM_DEVICE_TYPE  type;		/**<Type of device */
     int width;					/**<Width in bits*/
     int block_count;			/**<Total blocks. First block starts at 0. */
     int page_count;				/**<Page count per block*/
     int page_size;				/**<Number of bytes in a page including spare area*/
     int spare_size;				/**<Spare area size in bytes*/
    PLATFORM_DEVHANDLE handle;		/**<Handle to the block device as returned by Open. Handle is Opaque, do not interpret or modify */
     int	bboffset;				/**<Offset into spare area to check for a bad block */
	unsigned int column;				/**<Column for a NAND device */
	unsigned int flags;					/**<Flags is a copy of the flags that were used to open the device */
	void	*internal;				/**<Do not use. Used internally by the platform library */
    unsigned char *bblist;				/** <Bad Block list or NULL if device does not support one  */
...
PLATFORM_DEVICE_info *platform_device_open(unsigned int deviceid, unsigned int flags);
...
Platform_STATUS platform_uart_set_baudrate(unsigned int baudrate);
...
Platform_STATUS platform_uart_read(unsigned char *buf, unsigned int delay);
...
Platform_STATUS platform_uart_write(unsigned char chr);
...
Platform_STATUS platform_led(unsigned int led_id, PLATFORM_LED_OP operation, LED_CLASS_E led_class);
...
Platform_STATUS platform_delay(unsigned int usecs);
...
void platform_delaycycles(unsigned int cycles);
 ...
 * =============================================================================
 */
unsigned char *Osal_platformMalloc (unsigned int num_bytes, unsigned int alignment);
 ...
 * =============================================================================
 */
void Osal_platformFree (unsigned char *dataPtr, unsigned int num_bytes);
 ...
 */
extern unsigned int platform_errno;
...
extern unsigned int platform_init_return_code;
