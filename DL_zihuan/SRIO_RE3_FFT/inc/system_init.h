...
extern USER_PARA vxx_user_para_buffer[2];
...
extern unsigned char pdsch_pseudo_init[5][10][NUM_CODEWORD_2][PDSCH_PSEUDO_LENGTH];

extern unsigned int UEphyto719_pbch[4];
extern void para_init(CONFIG_PARA* config_para_ptr,CELL_PARA* cell_para_ptr);
extern void para_init_f(CONFIG_PARA* config_para_ptr,CELL_PARA* cell_para_ptr);
...
extern int tcp3d_lte_pdsch(USER_PARA *user_para_ptr,
...
extern unsigned int tb_buffer_total[10][NUM_CODEWORD_2 * ((MAX_TB_BLOCK_SIZE + ETHERNET_PACK_LEN - 1)/32)];

extern void rs_sequences_gen(unsigned char  *restrict rs_sequence_Output,/*最多20slot*3symbol*220char,存储的是实虚部共同索引*/
        unsigned short Ncell_id,
        unsigned char N_cp);
// lycan 2015-9-23 dci process 添加
extern const unsigned int VCC_BLOCK_SIZE_TABLE_DCI1C[32];
extern void prb_demapping_update_normalcp(
        unsigned char  CFI,
        signed int  subsfn,
        signed short* symbol_start,
        signed int* subcarrier_demap_num,
        signed int  prb_start,
        signed int  prb_num,
        unsigned char cell_bw_prb);
extern void prb_demapping_update_extendcp(
        unsigned char  CFI,
        signed int  subsfn,
        signed short* symbol_start,
        signed int* subcarrier_demap_num,
